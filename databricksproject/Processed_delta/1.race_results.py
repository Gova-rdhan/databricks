...
# MAGIC %sql
# MAGIC create database if not exists delta1db
# MAGIC location "abfss://delta1@covidapp3.dfs.core.windows.net/"

# COMMAND ----------

races_df = spark.read.format("delta").load("abfss://delta@covidapp3.dfs.core.windows.net/races")
circuits_df = spark.read.format("delta").load("abfss://delta@covidapp3.dfs.core.windows.net/circuits")
drivers_df = spark.read.format("delta").load("abfss://delta@covidapp3.dfs.core.windows.net/drivers").withColumnRenamed("name","driver_name")
constructors_df = spark.read.format("delta").load("abfss://delta@covidapp3.dfs.core.windows.net/constructors")

# COMMAND ----------

results_df = spark.read.format("delta").load("abfss://delta@covidapp3.dfs.core.windows.net/result").filter(f"file_date = '{date}'")
...
incremental_upsert('delta1db','race_results','delta1',final,'raceId','tgt.raceId = src.raceId')
