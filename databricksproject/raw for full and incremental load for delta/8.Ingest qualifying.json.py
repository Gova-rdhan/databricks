...
df = spark.read.option("multiline",True).schema(qualify_schema).json(f"abfss://rawdb@covidapp3.dfs.core.windows.net/{date}/qualifying")
...
incremental_upsert('deltadb','qualifying','delta',df1,'raceId','tgt.raceId = src.raceId')

# COMMAND ----------

# MAGIC %sql
# MAGIC select max(raceId) from deltadb.qualifying
